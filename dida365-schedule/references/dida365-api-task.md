# TickTick API - Task Creation

This document outlines the essential API details for creating tasks (schedules) in TickTick.

## Base URL
`https://api.ticktick.com/api/v2/`

## Authentication (OAuth 2.0)
To interact with the TickTick API, you need an access token obtained via OAuth 2.0. This skill assumes you have already obtained an access token.
It is recommended to store your `access_token` as an environment variable named `TICKTICK_ACCESS_TOKEN`.
...
```json
{
  ...
  "content": "Optional description of the task.",
  "desc": "Optional longer description.",
  "startDate": "YYYY-MM-DDTHH:MM:SSZ", // e.g., "2026-02-28T09:00:00+0800" or "2026-02-28T01:00:00.000+0000" (ISO 8601 format with timezone)
  "dueDate": "YYYY-MM-DDTHH:MM:SSZ",
  "timeZone": "Asia/Shanghai", // e.g., "Asia/Shanghai" or "America/New_York"
  "isAllDay": false,
  "priority": 0, // 0: None, 1: Low, 3: Medium, 5: High
  "reminders": [
    {
      "type": "trigger",
      "triggerId": "PT15M" // Remind 15 minutes before due date
    }
  ]
}
```

### Key Fields for Simple Schedule:
- `title` (string, required): The title of the task/schedule.

### Optional Fields:
- `projectId` (string): The ID of the project (list) the task belongs to. If not provided, it goes to the default inbox.
- `content` (string): A short description or notes for the task.
- `desc` (string): A longer, rich text description for the task.
- `startDate` (string, ISO 8601): The start date and time of the schedule.
- `dueDate` (string, ISO 8601): The due date and time of the schedule.
- `timeZone` (string): The timezone of the `startDate` and `dueDate`.
- `isAllDay` (boolean): Set to `true` for all-day events.
- `priority` (integer): Priority level (0=None, 1=Low, 3=Medium, 5=High).
- `reminders` (array of objects): Define reminders for the task.
...
---
**Note:** For more detailed information, refer to the official TickTick API documentation.
